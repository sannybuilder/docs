...
По умолчанию Sanny Builder читает конфигурацию режимов из файла `<SB>\data\modes.xml`. Этот путь может быть изменен через [опцию](../editor/cli.md#x) командной строки `-x`.
...
путь к JSON файлу, сгенерированному в [Sanny Builder Library](../editor/features.md#integration-with-sanny-builder-library), с документацией по скриптовым командам, используемым в целевой [игре](./#game).
...
Запуск Sanny Builder с [опцией](../editor/cli.md#x) `-x` позволяет загрузить конфигурацию режимов из файла, отличного от `modes.xml`. Если при этом Sanny Builder уже запущен, он перезагрузит конфигурацию и обновит список доступных режимов.
