...
Шаблоны - это заранее подготовленный код, которой вставляется в редактор, после ввода имени шаблона \(например, `load`\) и нажатия `F2`. Редактор вставит следующий код:
...
Все шаблоны содержатся в файле `templates.txt`. Этот файл имеет следующий синтаксис:
...
Также можно добавить новый шаблон в файл `templates.txt` прямо из редактора. Для этого нужно выделить нужный кусок текста и выбрать меню `Сервис->Добавить шаблон`. В появившемся диалоговом окне вы должны ввести имя шаблона \(также можно добавить описание\) и нажать кнопку `OK`. Новый шаблон будет сразу же готов к использованию.
